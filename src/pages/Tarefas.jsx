...
import { format, addDays, isToday, isPast, parseISO, getDay, nextDay, formatDistanceToNowStrict } from 'date-fns';
...
export default function Tarefas() {
  ...
  async function carregarTarefas() {
    ...
    if (error) { console.error("Erro ao carregar tarefas:", error.message); } else {
      const hoje = new Date();
      hoje.setHours(0,0,0,0); // Normaliza para comparar só a data
      setTarefas(data || []);
      if(isAtivo) {
  ...
  return (
    <>
      ...
      {modoExibicao === 'ativas' && (
        <div className="mb-10">
          <h2 className="text-xl font-bold mb-4 text-gray-700">🎯 Para Hoje</h2>
          <div className="bg-white p-4 rounded-lg shadow-md space-y-3">
            {tarefasHoje.length > 0 ? (
              tarefasHoje.map(tarefa => {
                ...
                const dataVencimento = parseISO(tarefa.due_date);
                const estaAtrasada = isPast(dataVencimento) && !isToday(dataVencimento);
                const tempoAtraso = estaAtrasada 
                  ? formatDistanceToNowStrict(dataVencimento, { addSuffix: true, locale: ptBR }) 
                  : "";

                return (
                  <div key={tarefa.id} className="flex justify-between items-center p-2 rounded-md hover:bg-gray-50">
                    <div className="flex items-center gap-3">
                      <span className="text-xl" title={prioridadesMap[tarefa.prioridade]?.titulo}>{prioridadesMap[tarefa.prioridade]?.icon}</span>
                      <div>
                         {/* ✅ NOME DA TAREFA COM COR CONDICIONAL */}
                        <p className={clsx("font-medium", estaAtrasada ? "text-red-600" : "text-gray-800")}>
                          {tarefa.nome}
                        </p>
                        {/* ✅ MENSAGEM DE ATRASO */}
                        {estaAtrasada && (
                          <span className="text-xs text-red-500">Atrasada {tempoAtraso}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={() => handleConcluirTarefa(tarefa)} className="p-1.5 text-green-500 hover:bg-green-100 rounded-full" title="Concluir"><CheckIcon className="h-5 w-5"/></button>
                      <button onClick={() => setEditando(tarefa)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-full" title="Editar"><PencilIcon className="h-5 w-5"/></button>
                    </div>
                  </div>
                );
              })
            ) : (
              <p className="text-sm text-gray-400 italic p-2">Nenhuma tarefa para hoje.</p>
            )}
          </div>
        </div>
      )}
